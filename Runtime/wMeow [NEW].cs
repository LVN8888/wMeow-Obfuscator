...
using ConversionBack.ExceptionHandlers;
using System;
...
namespace ConversionBack
{
    public class wMeow
    {
        ...
        public static object _____(Module module, MethodBase callingMethod, int ID, object[] parameters, byte[] bytes)
        {
            ...
            DynamicMethod dynamicMethod = new DynamicMethod("wwMeoww", callingMethod.MemberType == MemberTypes.Constructor ? null : ((MethodInfo)callingMethod).ReturnParameter.ParameterType, parametersArray, Initialize.callingModule, true);//create the dynamic method
        ...
        public static object Meoww(string text, string show2, RuntimeTypeHandle type, object[] parameters) //string text, string texte, object[] parameters, object obj, string show, string show2
        {
            string dec2 = Decode(Xoring(show2));

            string[] hey = Decode(Xoring2(text)).Split('-');

            int id = Convert.ToInt32(dec2); //new

            int size = Convert.ToInt32(hey[0]);

            int pos = Convert.ToInt32(hey[1]);
            ...
            else
            {
                ...
                var grabbedBytes = byteArrayGrabber(Initialize.byteArrayResource, pos, size);
                var decryptionKey = Decrypt2(MD5.Create().ComputeHash(Encoding.ASCII.GetBytes(callingMethod.Name)), Encoding.ASCII.GetBytes("Bytes"));
                var ab = callingMethod.GetMethodBody().GetILAsByteArray();
                Initialize.Run(grabbedBytes, grabbedBytes.Length, ab, ab.Length);


                return _____(Type.GetTypeFromHandle(type).Module, callingMethod, id, parameters, Decrypt(decryptionKey, grabbedBytes));//if not convert the method to a dynamic method
            }
        }
        ...
        {
            string decode = str.Replace("?", "4");
            decode = decode.Replace("%", "f");
            decode = decode.Replace(":", "3");
            decode = decode.Replace("-", "d");
            decode = decode.Replace("/", "2");
            decode = ConvertHexToString(decode);

            decode = decode.Replace("-", "3");
            decode = decode.Replace("/", "6");
            decode = ConvertHexToString(decode);
        ...
        public static byte[] Decrypt2(byte[] data, byte[] Keys)
        {
