...
namespace Core.Protection
{
    internal class ConvertToBytes
    {
        ...
        public void ConversionMethod()
        {
            ...
            ExceptionStarter();//Process Method Exception handlers.
            ...
            foreach (var ins in _methods.Body.Instructions) //loop all instructions in method
            {
                ...
                switch (opType)
                {
                    ...
                    case OperandType.InlineMethod:
                        ...
                        {
                            var methodSpec = ins.Operand as MethodSpec; //cast the operand to a methoddef(if included in current module) or methodref (if in an external module)
                            if (methodSpec == null)
                        ...
                        {
                            var methodDeforRef = ins.Operand as IMethodDefOrRef; //cast the operand to a methoddef(if included in current module) or methodref (if in an external module)
                    ...
                    case OperandType.InlineString:
                        ...
                        var operand = ins.Operand.ToString(); //get the instructions operand
                    ...
                    case OperandType.InlineI:
                        ...
                        var operand1 = ins.GetLdcI4Value(); //get the instructions operand as an integer since inlineI is ints
                        _binWriter.Write(operand1); //write string into binaryWriter.
                        continue;
                    case OperandType.ShortInlineVar:
                        _binWriter.Write((byte)4); //write 4 so when converting back i know its a shortInlineVar
                        if (ins.Operand is Local)
                        {
                            var loc = ins.Operand as Local; //cast it to a local(variable)
                            _binWriter.Write(loc.Index); //write its index so can refer to it in conversion back
                            _binWriter.Write((byte)0);
                        }
                        else if (ins.Operand is Parameter)
                        {
                            var par = ins.Operand as Parameter; //cast it to a local(variable)
                    ...
                    case OperandType.InlineField:
                        ...
                        {
                            var memberRef = ins.Operand as MemberRef; //cast the operand to a fielddef
                            if (memberRef == null)
                                throw new Exception("Check the instruction. This should not happen");
                            var mdToken1 = memberRef.MDToken.ToInt32();
                            _binWriter.Write(mdToken1);
                        }
                        else
                        {
                            var fieldDef = ins.Operand as FieldDef; //cast the operand to a fielddef
                            if (fieldDef == null)
                                throw new Exception("Check the instruction. This should not happen");
                            var mdToken1 = fieldDef.MDToken.ToInt32();
                            _binWriter.Write(mdToken1);
                        }
                        continue;
                    case OperandType.InlineType:
                        _binWriter.Write((byte)6); //write 6 so when converting back i know its a inline type
                        var typeDeforRef = ins.Operand as ITypeDefOrRef; //cast the operand to a typeDef or typeRef
                        if (typeDeforRef == null)
                            throw new Exception("Check the instruction. This should not happen");
                        var mdToken2 = typeDeforRef.MDToken.ToInt32();
                        _binWriter.Write(mdToken2);
                        continue;
        ...
        private void ExceptionStarter()
        {
            ...
            foreach (ExceptionHandler bodyExceptionHandler in _methods.Body.ExceptionHandlers)
            {
                ...
                var imported = Protector.moduleDefMD.Import(catchResolveTypeDef).ResolveTypeDef();
